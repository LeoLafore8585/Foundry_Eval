...
contract SimpleVotingSystemTest is Test {
    ...
    function _enterVotePhaseAfterDelay() internal {
        vm.startPrank(OWNER);
        votingSystem.setWorkflowStatus(SimpleVotingSystem.WorkflowStatus.VOTE);
        vm.stopPrank();

        // avancer le temps d'au moins 1 heure
    ...
    function test_AddCandidate_MultipleCandidates() public {
        ...
        SimpleVotingSystem.Candidate memory candidate1 = votingSystem
            .getCandidate(1);
        assertEq(candidate1.name, "Alice");
        assertEq(candidate1.id, 1);

        SimpleVotingSystem.Candidate memory candidate2 = votingSystem
            .getCandidate(2);
        assertEq(candidate2.name, "Bob");
        assertEq(candidate2.id, 2);

        SimpleVotingSystem.Candidate memory candidate3 = votingSystem
            .getCandidate(3);
    ...
    function test_Vote_InvalidCandidateId_TooHigh_WithMultipleCandidates()
        public
    {
        vm.startPrank(OWNER);
        votingSystem.addCandidate("Alice");
    ...
    function test_CompleteVotingScenario() public {
        ...
        SimpleVotingSystem.Candidate memory alice = votingSystem.getCandidate(
            1
        );
        ...
        SimpleVotingSystem.Candidate memory charlie = votingSystem.getCandidate(
            3
        );
    ...
    function test_CandidatesMapping_Public() public {
        ...
        (uint id1, string memory name1, uint voteCount1) = votingSystem
            .candidates(1);
        assertEq(id1, 1);
        assertEq(name1, "Alice");
        assertEq(voteCount1, 0);

        (uint id2, string memory name2, uint voteCount2) = votingSystem
            .candidates(2);
        assertEq(id2, 2);
        assertEq(name2, "Bob");
    ...
    function test_Vote_Revert_WhenNotInVotePhase() public {
        vm.startPrank(OWNER);
        votingSystem.addCandidate("Alice");
        vm.stopPrank();
    ...
    function test_Vote_Revert_BeforeOneHourDelay() public {
        vm.startPrank(OWNER);
        votingSystem.addCandidate("Alice");
        vm.stopPrank();
        ...
        vm.startPrank(voter1);
        vm.expectRevert("Voting not allowed yet");
        votingSystem.vote(1);
        vm.stopPrank();
    }

    function test_NFT_Minted_On_Vote() public {
        vm.startPrank(OWNER);
        votingSystem.addCandidate("Alice");
        vm.stopPrank();
        ...
        IVotingNFT nft = IVotingNFT(address(votingSystem.votingNFT()));

        assertEq(nft.balanceOf(voter1), 1);
    }

    function test_FundCandidate_And_Withdraw_AfterCompleted() public {
        vm.startPrank(OWNER);
        votingSystem.addCandidate("Alice");
        vm.stopPrank();
        ...
        assertEq(address(votingSystem).balance, amount);
        ...
        vm.startPrank(OWNER);
        votingSystem.withdraw(receiver, amount);
        vm.stopPrank();
    ...
    function test_GetWinningCandidateId_AfterCompleted() public {
        vm.startPrank(OWNER);
        votingSystem.addCandidate("Alice"); // id 1
        votingSystem.addCandidate("Bob"); // id 2
        vm.stopPrank();
        ...
        uint winningId = votingSystem.getWinningCandidateId();
        assertEq(winningId, 1);
    }

    function test_GetWinningCandidateId_Revert_IfNotCompleted() public {
        vm.startPrank(OWNER);
        votingSystem.addCandidate("Alice");
        vm.stopPrank();

        vm.expectRevert("Workflow not completed");
        votingSystem.getWinningCandidateId();
    }
}
